...
public class FileSystemTest {
    ...
    @Test
    public void testSortRootDirsByAlphabeticalOrderWithLambda() {
        List<FSElement> roots = new LinkedList<>(fs.getRootDirs());
        roots.sort((e1, e2) -> e1.getName().compareToIgnoreCase(e2.getName()));
    ...
    @Test
    public void testSortRootDirsByReverseAlphabeticalOrderWithLambda() {
        List<FSElement> roots = new LinkedList<>(fs.getRootDirs());
        roots.sort((e1, e2) -> e2.getName().compareToIgnoreCase(e1.getName()));
        for (int i = 1; i < roots.size(); i++) {
            assertTrue(roots.get(i - 1).getName().compareToIgnoreCase(roots.get(i).getName()) >= 0);
        }
    }
    ...
    @Test
    public void testSortRootDirsByCreationTimeWithLambda() {
        List<FSElement> roots = new LinkedList<>(fs.getRootDirs());
        roots.sort((e1, e2) -> e1.getCreationTime().compareTo(e2.getCreationTime()));
        for (int i = 1; i < roots.size(); i++) {
